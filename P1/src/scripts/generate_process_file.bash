...
do
	...
	if [ "$process_type" -le $ptype_def_bound ]; then
	    echo "        <ProcessType>RealTime</ProcessType>" >> $file_name
	else
	    echo "        <ProcessType>RoundRobin</ProcessType>" >> $file_name
	fi
	...
	do
		echo "            <Resource>" >> $file_name
		
		if [ "$resource_type" -eq 0 ]; then
			echo "                <ResourceType>CPU</ResourceType>" >> $file_name
			resource_type=1
		else
			echo "                <ResourceType>IO</ResourceType>" >> $file_name
			resource_type=0
		fi
